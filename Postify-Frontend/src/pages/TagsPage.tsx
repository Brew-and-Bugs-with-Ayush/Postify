...
const TagsPage: React.FC<TagsPageProps> = ({ isAuthenticated }) => {
  ...
  // ✅ Improved handleAddTags: collects both typed input and newTags
  const handleAddTags = async () => {
    let tagsToAdd = [...newTags];

    if (tagInput.trim() !== "") {
      const value = tagInput.trim().toLowerCase();
      if (!tagsToAdd.includes(value)) {
    ...
    try {
      ...
      await apiService.createTags(tagsToAdd);
      await fetchTags();
      handleModalClose();
    } catch (err) {
  ...
  // ✅ still support Enter/comma shortcuts
        ...
        <ModalContent>
          ...
          <ModalBody>
            <div className="space-y-4">
              <Input
                label="Enter tags"
                placeholder="Type and press Enter, comma, or click Add"
                value={tagInput}
